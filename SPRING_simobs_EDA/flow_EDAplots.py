...
def plotRegression1(df,FlowStationInfo):
    ...
    # position of text for slope and intercept
    text_left = 100
    text_top = -100
    if len(x_cal)>3:
        ...
        text_left = min([min(x_cal),text_left])
        text_top = max([max(y_cal),text_top])
    if len(x_ver)>3:
        ver_model,yp_ver = regRobust(x_ver,y_ver)
        ver_intercept = ver_model.params['const']
        ver_slope = ver_model.params[w]
        text_left = min([text_left,min(x_ver)])
        text_top = max([text_top,max(y_ver)])
    ...
    if len(x_cal)>3:
        ...
        if len(x_ver)>3:
            g.ax_joint.text(text_left, text_top
                , f"\ncal_slope={cal_slope:.3f}\ncal_intercept={cal_intercept:.5f}"
                + f"\nver_slope={ver_slope:.3f}\nver_intercept={ver_intercept:.5f}"
                , fontsize=10)
            with open("flow_regression_params.csv", "a") as f:
...
if __name__ == '__main__':
    ...
    with open("flow_regression_params.csv", "w") as f:
        f.write(f"ID,Cal_Slope,Cal_Intercept,Ver_Slope,Ver_Intercept\n") 
